	...
	for(i=0;i<product.length;i++){
		...
		div.innerHTML="<div class='image-box'><img src='"+product[i]["image"]+"'><div class='heart'><p><i class='fa fa-heart'></i></p></div><div class='cond'><p>Condition</p></div></div><div class='refurbish'>"+brand+" Refurbished - Superb</div><div class='title'>"+product[i]["title"]+"</div><div class='price'><div class='current-price'>$"+currentPrice+"</div><div class='actual-price'>$<del>"+product[i]["price"]+"</del></div><div class='offer'>"+randOffer+"% off</div></div>";
		cart.appendChild(div);
	}
	var heart = document.querySelectorAll('.heart');
	var heartI = document.querySelectorAll('.heart i');
	...
	sort_div.className="sort-box";
	sort_div.innerHTML="<li>Default</li><li>Lower to upper price</li><li>Upper to lower price</li>";
	sort.appendChild(sort_div);

	var filter_div=document.createElement("div");
	filter_div.className="filter-box";
	filter_div.innerHTML="<li>Default</li><li>Price $1 - $20</li><li>Price $21 - $50</li><li>Price $51 - $100</li><li>Price $101 - $200</li><li>Price $201 - $500</li><li>Price $501 - $1000</li>";
	filter.appendChild(filter_div);

	sort.addEventListener("click",function(){		
		sort_div.classList.toggle("active");
	})
	filter.addEventListener("click",function(){		
		filter_div.classList.toggle("active");
	})

	const client = algoliasearch('6FNTKPDKX0','ee2a9435dec3e8883465253557300073');
	const products = client.initIndex('greenday');
	...
	autocomplete(
		'#search', {
			debug: true,
			templates: {
				dropdownMenu: '<div class="product"></div>',
			},
		}, [{
			source: autocomplete.sources.hits(products, { hitsPerPage: 20 }),
			displayKey: 'title',		
			templates: {
				header: '<div class="suggestions">Products</div>',
				suggestions(highlightResult) {
					return '<span class="sg-pro">'+highlightResult.title.value+'</span>';				
				},
				empty: '<div class="empty">No matching Product</div>'
			}
		}]
		);	
